...
namespace wuss {
int run(std::size_t argc, char* argv[])
{
...
void handle_help(const po::options_description& description_) noexcept {
    std::cout << "Usage: options_description [options]\n";
    std::cout << description_;
}

void handle_create_new_wallet() noexcept {
...
void handle_delete_wallet() noexcept {
...
void handle_new_entry() noexcept {
    if (!check_master_password()) {
        return 1;
    }
...
void handle_edit_entry() noexcept {
    std::cout << "Editing entry is not implemented\n";
}

void handle_view_entry() noexcept {
    if (!check_master_password()) {
        return 1;
    }
...
void handle_remove_entry() noexcept {
    if (!check_master_password()) {
        return 1;
    }
...
void handle_view_all_entries() noexcept {
    if (!check_master_password()) {
        return 1;
    }
...
void handle_change_master_password() noexcept {
    if (!check_master_password()) {
        return 1;
    }
